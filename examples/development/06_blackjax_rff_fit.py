# examples/06_blackjax_rff_fit.py
...
y_obs = jnp.squeeze(apply({"parameters": theta_true[None, :]})["predictions"], axis=0)
...
def loglik(theta):
    y_pred = jnp.squeeze(apply({"parameters": theta[None, :]})["predictions"], axis=0)
...
(_, _), samples = jax.lax.scan(one_step, (initial_state, rng_key), xs=None, length=400)
