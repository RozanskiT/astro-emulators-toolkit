"""Vendored SOAP_JAX implementation.
...
Minimal local edits:
- vendored under the toolkit package
- removed type-only dependencies on `chex` and `jaxtyping`
"""
...
class Preconditioner:
    ...
    @classmethod
    def tree_unflatten(
        cls, aux_data: None, children: tuple[PreconditionerMatrix, ...]
    ) -> "Preconditioner":
        return cls(children)

    def map(
        self, fn: Callable[[PreconditionerMatrix], PreconditionerMatrix]
    ) -> "Preconditioner":
...
def soap(
    ...
    mu_dtype: ArrayDType | None = None,
    qr_dtype: ArrayDType = jnp.float32,
) -> optax.GradientTransformationExtraArgs:
    """SOAP optimizer based on the public SOAP_JAX implementation."""
...
def scale_by_soap(
    ...
    """SOAP gradient transformation based on the public SOAP_JAX implementation."""
    ...
    def update_step(
        ...
        exp_avg_sq = otu.tree_update_moment_per_elem_norm(
        ...
        norm_updates = jtu.tree_map(
            lambda e_avg, e_avg_sq, q: project_back(
                e_avg / (jnp.sqrt(e_avg_sq) + jnp.asarray(eps, dtype=e_avg_sq.dtype)),
                q,
                precision,
            ),
            exp_avg,
        ...
        if correct_bias:
            ...
            norm_updates = jtu.tree_map(
                lambda p: p * jnp.asarray(corr, dtype=p.dtype), norm_updates
            )
        ...
        def refresh_preconditioner() -> tuple[Updates, Updates, Updates]:
            new_Q_and_exp_avg_sq = jtu.tree_map(
                lambda e, gg, q: get_orthogonal_matrix_QR(
                    gg, q, e, precision, qr_dtype
                ),
                exp_avg_sq,
                new_GG,
                state.Q,
            ...
            new_exp_avg = jtu.tree_map(
                lambda e, old_q, new_q: project(
                    project_back(e, old_q, precision), new_q, precision
                ),
                exp_avg,
                state.Q,
                new_Q,
    ...
    def update_fn(
        updates: Updates, state: SOAPState, params: Updates | None = None
    ) -> tuple[Updates, SOAPState]:
        del params
    ...
    return optax.GradientTransformation(init_fn, update_fn)  # type: ignore[return-value]
...
def add_decayed_weights_post(
    ...
    def update_fn(
        updates: Updates,
        state: PostDecayState,
        params: Updates | None = None,
    ...
    if grad.ndim == 1:
        ...
        return Preconditioner(
            [
                lerp(
                    GG.matrices[0],
                    jnp.matmul(grad[:, None], grad[None, :], precision=precision),
                    1 - beta,
                )
            ]
        )
...
def get_orthogonal_matrix(gg: Array | None, qr_dtype: ArrayDType) -> Array | None:
    if gg is None:
        return None
...
def _resolve_learning_rate(
    learning_rate: optax.ScalarOrSchedule, count: Array
) -> Array:
